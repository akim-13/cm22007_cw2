...
def get_all_events(username: str, db: Session) -> dict:
    ...
    if user is None:
        return {"events": []}
    events: list[Event] = user.events
    events = [{"start": event.start.strftime(DATETIME_FORMAT), "end": event.end.strftime(DATETIME_FORMAT), "eventID": event.eventID, "title": event.task.title} for event in events]
