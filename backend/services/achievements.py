...
def update_from_user(username: str, db: Session) -> dict[str, bool]:
    ...
    unachieved = db.query(Achievements).filter(~Achievements.achievementID.in_(user_achievements))
...
def get_from_user(username: str, db: Session) -> dict[str, list[dict]]:
    ...
    achievements = (
        db.query(Achievements_to_User).filter(Achievements_to_User.username == username).all()
    )
