...
from services import tasks_service, user_service, task_scheduler, event_service
...
def test_adding_tasks(db_session):
    ...
    tasks = tasks_service.get_user_tasks(username, db_session).get("tasks")
...
def test_deleting_tasks(db_session):
    ...
    # delete task1
    assert tasks_service.delete_task(task1.taskID, db_session).get("task_deleted")
    tasks = tasks_service.get_user_tasks(username, db_session).get("tasks") 
    assert tasks is not None
    assert len(tasks) == 1
    assert tasks[0]["title"] == task2.title  # task2 should only be in the table now
    
    assert not tasks_service.delete_task(10, db_session).get("task_deleted") # Task with task id == 10 doesn't exist
...
def test_edit_task(db_session):
    ...
    assert tasks_service.edit_task(task1.taskID, dict(title="CS Coursework", duration=1200), db_session).get("success")
    tasks = tasks_service.get_user_tasks(username, db_session).get("tasks")
    assert tasks[0]["title"] == "CS Coursework" 
    assert tasks[0]["duration"] == 1200
    
    # editing task that doesn't exist
    assert not tasks_service.edit_task(10, dict(title="CS coursework"), db_session).get("success")
    
    # editing fields that don't exist on a task that does exist
    assert not tasks_service.edit_task(task1.taskID, dict(humidity=23), db_session).get("success")
    
    # editing a valid field, but with wrong datatype
    assert not tasks_service.edit_task(task1.taskID, dict(title=23), db_session).get("success")
    tasks = tasks_service.get_user_tasks(username, db_session).get("tasks")
    assert tasks[0]["title"] == "CS Coursework"
    
    # editing the primary key of the task (shouldn't allow)
    assert not tasks_service.edit_task(task1.taskID, dict(taskID=23), db_session).get("success")
    tasks = tasks_service.get_user_tasks(username, db_session).get("tasks")
...
def test_task_completion(db_session):
    ...
    tasks_service.set_task_complete(task1.taskID, db_session)
    tasks_service.set_task_complete(task2.taskID, db_session)
    points = user_service.get_user_points(username, db_session).get("points")
    assert points == task1.duration + task2.duration
    
    # Set task 2 to be incomplete and check points again
    tasks_service.set_task_incomplete(task2.taskID, db_session)
    points = user_service.get_user_points(username, db_session).get("points")
